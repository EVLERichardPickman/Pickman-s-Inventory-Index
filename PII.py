...
def get_app_icon():
    """
    Return a QIcon for the application.
    - Try extracting the icon from the running .exe (when frozen).
    - Fallback to an external `favicon.ico` located next to the script/.exe.
    Returns None if no icon can be loaded.
    """
    # First try embedded exe icon
    icon = get_exe_icon_qicon()
    if icon is not None:
        return icon

    # When frozen, try packaged favicon.ico from _MEIPASS (PyInstaller temp dir)
    try:
        if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
            ico_path_meipass = os.path.join(sys._MEIPASS, "favicon.ico")
            if os.path.exists(ico_path_meipass):
                return QIcon(ico_path_meipass)
    except Exception:
        pass

    # Fallback to local favicon.ico next to script/exe
    try:
...
class AnnexWindow(QMainWindow):
    def __init__(self):
        ...
        # Use embedded exe icon or fallback to favicon.ico
        icon = get_app_icon()
        if icon is not None:
...
def main():
    ...
    # Try to use embedded exe icon or fallback to favicon.ico
    icon = get_app_icon()
    if icon is not None:
